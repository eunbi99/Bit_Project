...
<!--상품 목록 - 관리자-->
 <section style="margin: 0 100px 0 100px ;">
  ...
  <table class="table" style="margin-top: 30px; font-size: 17px; border-top: 2px solid #333;">
    ...
    <c:forEach var="pList" items ="${pVo}">
    <tbody style="font-size: 15px">
    	<tr>
           <%--  <td><c:if test="${ pList.p_category == '육류' }">육류</c:if></td> --%>
            <td>${pList.p_category}</td>
            
            <td><img src="${pageContext.request.contextPath}/upload/${pList.p_file_upload}" style="height: 70px;"></td>
             
             <td><a href="productDetail?p_number=${pList.p_id }">${pList.p_id}</a>
            ...
            <td>${pList.p_count}</td>
           <%--  <td>
            <button style="margin-left:-6px; height:36px; font-size:10px;" id="edit" name="${pList.p_id }" onclick="edit(this)">수정
            </button>
            <button style="margin-left:-2px; height:36px; font-size:10px;" id="del" name="${pList.p_id }" onclick="del(this)">삭제
            </button>
            </td> --%>
